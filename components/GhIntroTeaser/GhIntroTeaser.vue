<template>
  <section class="gh-intro-teaser relative flex justify-center items-center h-screen overflow-hidden">
    ...
    <div class="gh-intro-teaser__fg-overlay absolute inset-0 flex justify-center lg:justify-end items-end min-w-full min-h-full">
      <div class="flex flex-col justify-center items-center lg:items-end p-8 lg:p-16 w-full transition-opacity">
        <span
...
import {
  ...
  NEXT_TEASER_SCROLL_DELAY_MS,
  TEXT_FADE_DURATION_MS,
  TEXT_ROTATION_DELAY_MS,
  VIDEO_FADE_DURATION_MS,
  VIDEO_LOOP_INTERVAL_MS,
  VIDEO_PLAYBACK_RATE,
...
export default class GhIntroTeaser extends Vue {
  ...
  public videoLoopInterval: number = VIDEO_LOOP_INTERVAL_MS;
  ...
  public onVideoMetaDataLoaded(video: HTMLVideoElement): void {
    ...
    const scrollToNextTeaserTimeout = (video.duration * 1000 + NEXT_TEASER_SCROLL_DELAY_MS) / parseFloat(VIDEO_PLAYBACK_RATE);
    ...
    setTimeout(() => {
      this.startUpdateTeaserTextKeyRunner();
      this.showTeaserText();
  ...
  public showTeaserText(): void {
    this.teaserTextDynamicClassList = `duration-${TEXT_FADE_DURATION_MS} opacity-100 text-custom-maximumYellow`;
  }

  public hideTeaserText(): void {
    this.teaserTextDynamicClassList = `duration-${TEXT_FADE_DURATION_MS} opacity-0 text-custom-maximumYellow`;
  }
  ...
  public startUpdateTeaserTextKeyRunner(): void {
    let lastIndex = this.updateTeaserTextKey(0);

    setInterval(() => {
      lastIndex = this.updateTeaserTextKey(lastIndex);
    }, TEXT_ROTATION_DELAY_MS);
  }

  public updateTeaserTextKey(lastIndex: number): number {
    let index = 0;

    do {
      index = Math.floor(Math.random() * this.teaserData.teaserText.length);
    } while (index === lastIndex)

    this.hideTeaserText();

    setTimeout(() => {
      this.teaserTextKey = this.teaserData.teaserText[index];
      this.updateTeaserText();
    }, TEXT_FADE_DURATION_MS);
